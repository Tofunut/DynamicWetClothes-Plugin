...
namespace
{
    ...
    int32 ResolveSurfaceNormalUVChannel(
        const int32 Selection,
        const int32 OriginalUVChannelIndex)
    {
        return Selection == INDEX_NONE ? OriginalUVChannelIndex : Selection;
    }

    FText BuildSurfaceNormalUVChannelLabel(
        const int32 Selection,
        const int32 OriginalUVChannelIndex)
    {
        ...
        {
            return FText::Format(
                LOCTEXT("SurfaceNormalUVSameAsOriginalLabel", "Same as Original (UV{0})"),
                FText::AsNumber(OriginalUVChannelIndex));
        }

        return FText::Format(
            LOCTEXT("SurfaceNormalUVExplicitLabel", "UV{0}"),
        ...
        const int32          Selection)
    {
        return IsExistingSourceUVChannel(
            Mesh,
            ResolveSurfaceNormalUVChannel(Selection, OriginalUVChannelIndex));
    }
        ...
        const int32          Selection)
    {
        ...
        const int32 ResolvedChannel = ResolveSurfaceNormalUVChannel(
            Selection,
            OriginalUVChannelIndex);
        if (!IsExistingSourceUVChannel(Mesh, ResolvedChannel))
        {
            return FText::Format(
                LOCTEXT(
                    "SurfaceNormalUVUnavailable",
                    "UV{0} is not available on LOD0. Choose the UV used by the material's tangent-space normal map."),
                FText::AsNumber(ResolvedChannel));
        }

        return LOCTEXT(
            "SurfaceNormalUVInfo",
            "Surface Water tangent-space normals use this UV. Normally keep Same as Original unless the material's normal map uses another channel.");
    }
...
void UWetClothingAssetCreationSettings::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
    ...
    if (PropertyName == GET_MEMBER_NAME_CHECKED(UWetClothingAssetCreationSettings, SourceSkeletalMesh))
    {
        ...
        SurfaceWaterNormalUVChannelIndex = INDEX_NONE;
