...
namespace
{
    ...
    constexpr float PreviewFixedStep = 1.0f / 60.0f;
    constexpr int32 PreviewMaxStepsPerFrame = 4;
    constexpr float PreviewRestartDebounce = 0.12f;
    constexpr float PreviewWaterRepeatInterval = 0.12f;
    ...
    uint32 BuildSimulationParameterHash(const FWetnessProfileParameters& Parameters)
    {
        ...
        AddValue(Surface.bUseSecondaryDroplets);
    ...
    const float InDeltaTime)
{
    ...
    if (PreviewBehavior == EPreviewBehavior::Simulation)
    {
        ...
        if (bSpaceAddWaterHeld && bHasPreviewWaterSelection && bPreviewSelectedChannelEnabled)
        {
            SpaceAddWaterRepeatAccumulator += FMath::Max(InDeltaTime, 0.0f);
            while (SpaceAddWaterRepeatAccumulator >= PreviewWaterRepeatInterval)
            {
                SpaceAddWaterRepeatAccumulator -= PreviewWaterRepeatInterval;
                ApplyPreviewSplash();
            }
        }
    }
    ...
    const FKeyEvent& InKeyEvent)
{
    ...
    {
        if (!bHasPreviewWaterSelection || !bPreviewSelectedChannelEnabled)
        {
            return FReply::Handled();
        }
        if (!bSpaceAddWaterHeld)
        {
            bSpaceAddWaterHeld = true;
            SpaceAddWaterRepeatAccumulator = 0.0f;
            ApplyPreviewSplash();
        }
        return FReply::Handled();
    }
    ...
    const bool bSelectedChannelEnabled)
{
    ...
    bPreviewSurfaceSelection = bHasSelection && bSurfaceSelected;
    bPreviewSecondarySelection = bPreviewSurfaceSelection && bSecondarySelected;
    bPreviewSelectedChannelEnabled = bHasSelection && bSelectedChannelEnabled;
    if (ViewportClient.IsValid())
    {
        ViewportClient->Invalidate();
    }
...
void SWetnessProfileViewport::ApplyPreviewSplash()
{
    ...
    // Apply immediately even while paused, then rebind the newly selected ping-pong RT.
    GPUPreviewSimulator->Step(PreviewFixedStep, PreviewAnimationTime);
    PreviewAnimationTime = FMath::Min(PreviewAnimationTime + PreviewFixedStep, GetPreviewLoopDuration());
    BindGPUPreviewTextures();
    if (ViewportClient.IsValid())
    {
        ViewportClient->Invalidate();
    ...
    Overlay->AddSlot()
        ...
        [
            ...
            [
                ...
                .Font(FCoreStyle::GetDefaultFontStyle(TEXT("Bold"), 44))
                .ColorAndOpacity(FSlateColor(FLinearColor(1.0f, 0.10f, 0.08f, 1.0f)))
        ...
        [
            ...
            [
                ...
                [
                    SNew(SBorder)
                    .BorderImage(FCoreStyle::Get().GetBrush(TEXT("WhiteBrush")))
                    .BorderBackgroundColor(FLinearColor(0.15f, 0.16f, 0.18f, 1.0f))
                    .Padding(FMargin(7.0f, 2.0f))
                    [
                        SNew(STextBlock)
                        .Text(LOCTEXT("SpaceKeyLabel", "Space"))
                ...
                [
                    SNew(STextBlock)
                    .Text(LOCTEXT("HoldSpaceToAddWater", "Hold to Add Water"))
...
void SWetnessProfileViewport::InitializePreviewComponents()
{
    ...
    PreviewSkeletalMeshComponent->SetEnableAnimation(false);
    PreviewSkeletalMeshComponent->SetForceRefPose(true);
...
{
    FVector2f CameraCenterUV;
    if (TryResolveCameraCenterSplashUV(CameraCenterUV))
    {
        PreviewScenarioSplashUV = CameraCenterUV;
    }
    else
    {
        // Keep a visible fallback for the engine sphere or when the camera misses the mesh.
...
void SWetnessProfileViewport::TickGPUPreviewSimulation(const float InDeltaTime)
{
    ...
    while (PreviewSimulationAccumulator >= PreviewFixedStep && SafetyCounter++ < PreviewMaxStepsPerFrame)
    {
        ...
        // Step swaps the simulator's Current/Next render targets. Rebind immediately so
        // the preview material never keeps sampling the previous ping-pong target.
        BindGPUPreviewTextures();
        bStepped = true;
    }
...
FText SWetnessProfileViewport::GetOverlayText() const
{
    ...
    {
        return LOCTEXT("SimulationPreviewUnavailable", "GPU Simulation · Unavailable");
    }

    const FText StateText = bPreviewAnimationEnabled
        ? LOCTEXT("SimulationPreviewPlayingState", "PLAYING")
        : LOCTEXT("SimulationPreviewPausedState", "PAUSED");

    if (!bHasPreviewWaterSelection)
    {
        return LOCTEXT("SimulationPreviewSelectType", "SELECT A WATER TYPE");
    }
    ...
    if (!bPreviewSelectedChannelEnabled)
    {
        ...
        {
            return bPreviewSecondarySelection
                ? LOCTEXT("SimulationPreviewSurfaceSecondaryDisabled", "SURFACE WATER · SECONDARY · OFF")
                : LOCTEXT("SimulationPreviewSurfacePrimaryDisabled", "SURFACE WATER · PRIMARY · OFF");
        }
        return LOCTEXT("SimulationPreviewAbsorbedDisabled", "ABSORBED WATER · OFF");
    }

    if (!bPreviewSurfaceSelection)
    {
        return FText::Format(
            LOCTEXT("SimulationPreviewAbsorbedFormat", "ABSORBED WATER · {0}"),
            StateText);
    }

    const FText SurfaceLayerText = bPreviewSecondarySelection
        ? LOCTEXT("SimulationPreviewSecondaryLayer", "SECONDARY")
        : LOCTEXT("SimulationPreviewPrimaryLayer", "PRIMARY");
    return FText::Format(
        LOCTEXT("SimulationPreviewSurfaceFormat", "SURFACE WATER · {0} · {1}"),
        SurfaceLayerText,
        StateText);
}
