...
namespace
{
        ...
        const int32 UVChannelIndex)
    {
        ...
        return WetClothingAsset->Derived.Inline.BakedWetnessProfileMaps.FindByPredicate(
    ...
    FString* OutSummary)
{
    ...
    {
        const FWetClothingGeneratedWetMaterialOverride* MaterialOverride = WetClothingAsset->Derived.Inline.GeneratedWetMaterialOverrides.FindByPredicate(
        ...
        else if (MaterialOverride->SourceMaterial != Materials[MaterialSlotIndex].MaterialInterface)
        {
            PendingLines.Add(FString::Printf(TEXT("Material setup source changed for slot %d."), MaterialSlotIndex));
        ...
        else if (!FWCAMaterialGenerator::IsMaterialConfiguredForDwc(
                 ...
                 !FWCAMaterialGenerator::IsMaterialConfiguredForDwc(
                     ...
                     FWCAMaterialGenerator::MakeOptionsForAsset(WetClothingAsset, EDWCSimulationMode::WetnessMapGPU)))
        {
    ...
    for (const FWetClothingBakedWetnessProfileMap& BakedWetnessProfileMap : WetClothingAsset->Derived.Inline.BakedWetnessProfileMaps)
    {
    ...
    bool* OutHadWarnings)
{
    ...
    const int32 RemovedOverrideCount = WetClothingAsset->Derived.Inline.GeneratedWetMaterialOverrides.RemoveAll(
    ...
    for (const int32 MaterialSlotIndex : WetMaterialSlots)
    {
        ...
        FWetClothingGeneratedWetMaterialOverride* ExistingOverride = WetClothingAsset->Derived.Inline.GeneratedWetMaterialOverrides.FindByPredicate(
        ...
        {
            ExistingOverride = &WetClothingAsset->Derived.Inline.GeneratedWetMaterialOverrides.AddDefaulted_GetRef();
    ...
    for (int32 MapIndex = bCanPruneStaleWetnessProfileMaps ? WetClothingAsset->Derived.Inline.BakedWetnessProfileMaps.Num() - 1 : INDEX_NONE; MapIndex >= 0; --MapIndex)
    {
        const FWetClothingBakedWetnessProfileMap& BakedWetnessProfileMap = WetClothingAsset->Derived.Inline.BakedWetnessProfileMaps[MapIndex];
        const FString TextureUvKey = MakeTextureUvKey(BakedWetnessProfileMap.SourceTexture.Get(), BakedWetnessProfileMap.UVChannelIndex);
        ...
        WetClothingAsset->Modify();
        WetClothingAsset->Derived.Inline.BakedWetnessProfileMaps.RemoveAt(MapIndex);
        WetClothingAsset->MarkPackageDirty();
    }
