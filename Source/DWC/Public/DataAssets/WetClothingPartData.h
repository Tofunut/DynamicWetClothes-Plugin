...
struct DWC_API FWetPartProfileAssignment
{
    ...
    /** Typed soft reference used by new code. It survives Content Browser move/rename operations. */
    ...
    {
        return GetSourceProfilePath().IsValid();
    }
    ...
    {
        const FSoftObjectPath Path = GetSourceProfilePath();
        return Path.IsValid() ? Path.GetAssetName() : FString();
    }
};
...
struct DWC_API FWetClothingEditableWetPartData
{
        ...
        const EWetPartProfileBlendMode   BlendMode = EWetPartProfileBlendMode::Standard)
    {
        ...
        const int32 ExistingIndex = Profiles.IndexOfByPredicate(
            ...
            {
                return Candidate.GetSourceProfilePath() == SourceProfile && Candidate.BlendMode == BlendMode;
            });
        ...
        Added.SetSourceProfilePath(SourceProfile);
