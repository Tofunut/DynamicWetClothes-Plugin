 ...
 *
 * Jobs reserve memory before Prepare captures immutable snapshots. A worker
 * may return a continuation with retained and next-phase estimates; the same
 * ticket and RAII lease then wait for exact phase admission without occupying
 * an active worker slot. Apply always runs on the game thread after generation
 * and authoring revision checks.
 */
class FDWCEditorWorkerJobScheduler final
    : public TSharedFromThis<FDWCEditorWorkerJobScheduler, ESPMode::ThreadSafe>
{
  public:
    using FWork = FDWCEditorWorkerPhaseWork;
  ...
  private:
    ...
    bool TryAdmitPendingPhase(const TSharedRef<FQueuedJob, ESPMode::ThreadSafe>& Job);
    ...
    TArray<TSharedRef<FQueuedJob, ESPMode::ThreadSafe>> PendingPhaseAdmissionJobs;
