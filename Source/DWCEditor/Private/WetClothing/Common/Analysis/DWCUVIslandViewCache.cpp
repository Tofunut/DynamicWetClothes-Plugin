...
namespace
{
    ...
    struct FDWCUVIslandViewCacheKey
    {
        FObjectKey MeshKey;
        FObjectKey AssetKey;
        int32 LODIndex = 0;
        ...
        bool operator==(const FDWCUVIslandViewCacheKey& Other) const
        {
    ...
    uint32 GetTypeHash(const FDWCUVIslandViewCacheKey& Key)
    {
    ...
    struct FDWCUVIslandViewCacheEntry
    {
        TArray<FWetClothingAssetUVIsland> Islands;
    };

    TMap<FDWCUVIslandViewCacheKey, FDWCUVIslandViewCacheEntry> GUVIslandCache;

    void CopyCachedIslands(
        const FDWCUVIslandViewCacheEntry& Entry,
...
bool FDWCUVIslandViewCache::GetMaterialSlotUVIslands(
    ...
    FString*                                       OutErrorMessage)
{
    ...
    const FDWCUVIslandViewCacheKey Key(
        SkeletalMesh,
        nullptr,
        LODIndex,
        UVChannelIndex,
    ...
    if (const FDWCUVIslandViewCacheEntry* ExistingEntry = GUVIslandCache.Find(Key))
    {
        CopyCachedIslands(*ExistingEntry, OutIslands);
        return true;
    }

    FDWCUVIslandViewCacheEntry NewEntry;
...
bool FDWCUVIslandViewCache::BuildMaterialSlotPreviewTriangles(
    const USkeletalMesh*                 SkeletalMesh,
    int32                                MaterialSlotIndex,
...
void FDWCUVIslandViewCache::InvalidateAll()
{
    GUVIslandCache.Reset();
}

void FDWCUVIslandViewCache::InvalidateMesh(const USkeletalMesh* SkeletalMesh)
{
    if (SkeletalMesh == nullptr)
    {
        return;
...
void FDWCUVIslandViewCache::InvalidateAsset(const UWetClothingAsset* WetClothingAsset)
{
    if (WetClothingAsset == nullptr)
    {
        return;
