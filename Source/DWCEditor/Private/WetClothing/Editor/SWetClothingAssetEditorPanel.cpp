...
#include "WetClothing/WetPart/WetPartAutoPartitioner.h"
...
#include "WetClothing/Widgets/SWetPartAutoPartitionControls.h"
                                       ...
                                       [SNew(SHorizontalBox)
                                        ...
                                        + SHorizontalBox::Slot()
                                              ...
                                              .Padding(4.0f, 0.0f, 0.0f, 0.0f)
                                                  [SNew(SButton)
                                                       .Text(LOCTEXT("BuildWetSetupButton", "Build"))
                                                       .ToolTipText(LOCTEXT("BuildWetSetupTooltip", "Create wet materials and bake missing Wetness Profile Maps for the current Wet Clothing Asset."))
                                                       .OnClicked(this, &SWetClothingAssetEditorPanel::HandleBuildWetSetupClicked)]

                                        + SHorizontalBox::Slot()
                                              .AutoWidth()
                                              .Padding(4.0f, 0.0f, 0.0f, 0.0f)
                                                  [SNew(SButton)
                                                       .Text(LOCTEXT("SaveAndBuildWetSetupButton", "Save & Build"))
                                                       .ToolTipText(LOCTEXT("SaveAndBuildWetSetupTooltip", "Build wet setup, then save the Wet Clothing Asset."))
                                                       .OnClicked(this, &SWetClothingAssetEditorPanel::HandleSaveAndBuildClicked)]]
                                                   ...
                                                   + SHorizontalBox::Slot()
                                                         ...
                                                         .VAlign(VAlign_Center)
                                                             [SNew(SButton)
                                                                  ...
                                                                  .HAlign(HAlign_Center)
                                                                  .VAlign(VAlign_Center)
                                                                  .OnClicked(this, &SWetClothingAssetEditorPanel::HandleAssignSelectedUVIslandToWetPartClicked)
                                                                      [SNew(STextBlock)
                                                                           .Text(this, &SWetClothingAssetEditorPanel::GetAssignUVIslandToWetPartText)
...
void SWetClothingAssetEditorPanel::EnsureDefaultWetPartForSelectedScope()
{
    ...
    const int32 UVChannelIndex = GetSelectedUVChannelIndex();
    for (FWetClothingAssetWetPartEntry& Entry : WetClothingAssetPtr->WetPartEntries)
    {
    ...
    WetClothingAssetPtr->WetPartEntries.Add(NewEntry);
    WetClothingAssetPtr->MarkPackageDirty();
    if (DetailsView.IsValid())
    {
        DetailsView->ForceRefresh();
...
const FWetClothingAssetWetPartEntry* SWetClothingAssetEditorPanel::FindWetPartEntryForUVIsland(int32 UVIslandID) const
{
    if (const UWetClothingAsset* WetClothingAssetPtr = WetClothingAsset.Get())
    {
        const int32 UVChannelIndex = GetSelectedUVChannelIndex();
...
const FWetClothingAssetWetPartEntry* SWetClothingAssetEditorPanel::FindEffectiveWetPartEntryForUVIsland(int32 UVIslandID) const
{
    if (const FWetClothingAssetWetPartEntry* AssignedEntry = FindWetPartEntryForUVIsland(UVIslandID))
    {
...
TSet<int32> SWetClothingAssetEditorPanel::GetUVIslandIDsForWetPart(int32 WetPartID) const
{
    TSet<int32> Result;
...
int32 SWetClothingAssetEditorPanel::GetEffectiveWetPartForUVIsland(int32 UVIslandID) const
{
    if (const FWetClothingAssetWetPartEntry* EffectiveEntry = FindEffectiveWetPartEntryForUVIsland(UVIslandID))
    {
...
TMap<int32, FLinearColor> SWetClothingAssetEditorPanel::BuildUVIslandColorMap() const
{
    ...
    for (const FUVIslandItemPtr& IslandItem : UVIslandItems)
    {
        ...
        if (const FWetClothingAssetWetPartEntry* Entry = FindEffectiveWetPartEntryForUVIsland(IslandItem->UVIslandID))
        {
...
TSet<int32> SWetClothingAssetEditorPanel::BuildHiddenUVIslandIDSet() const
{
    TSet<int32> Result;
...
TSharedRef<ITableRow> SWetClothingAssetEditorPanel::GenerateMaterialSlotRow(FMaterialSlotItemPtr Item, const TSharedRef<STableViewBase>& OwnerTable)
{
    ...
    if (const UWetClothingAsset* WetClothingAssetPtr = WetClothingAsset.Get())
    {
        SlotPreviewTriangles = SWetClothingAssetEditorPanelLocal::BuildMaterialSlotPreviewTriangles(WetClothingAssetPtr->TargetMesh, Item.IsValid() ? Item->SlotIndex : INDEX_NONE);
    }
        ...
        if (UWetClothingAsset* WetClothingAssetPtr = WetClothingAsset.Get())
        {
            if (const USkeletalMesh* TargetMesh = WetClothingAssetPtr->TargetMesh)
            {
                ...
                if (AssignedSlotIndices.Num() > 0)
                {
                    ...
                    {
                        FWetClothingAssetWetMaterialOverride* ExistingOverride = WetClothingAssetPtr->WetMaterialOverrides.FindByPredicate(
                            [MaterialIndex](const FWetClothingAssetWetMaterialOverride& MaterialOverride)
                            {
                        ...
                        {
                            ExistingOverride = &WetClothingAssetPtr->WetMaterialOverrides.AddDefaulted_GetRef();
...
TSharedRef<ITableRow> SWetClothingAssetEditorPanel::GenerateUVIslandRow(FUVIslandItemPtr Item, const TSharedRef<STableViewBase>& OwnerTable)
{
    ...
    if (Item.IsValid())
    {
        if (const FWetClothingAssetWetPartEntry* EffectiveEntry = FindEffectiveWetPartEntryForUVIsland(Item->UVIslandID))
        {
...
void SWetClothingAssetEditorPanel::HandleUVIslandSelectionChangedFromUVView(const TArray<int32>& UVIslandIDs, EWetClothingAssetUVSelectionOp SelectionOp)
{
    ApplyIslandSelection(UVIslandIDs, SelectionOp == EWetClothingAssetUVSelectionOp::Add);
}

void SWetClothingAssetEditorPanel::HandleUVIslandPickedFromPreview(int32 UVIslandID, bool bAppendSelection)
{
...
void SWetClothingAssetEditorPanel::ApplyIslandSelection(const TArray<int32>& HitUVIslandIDs, bool bAppendSelection)
{
...
void SWetClothingAssetEditorPanel::SetSelectedUVIslandIDs(const TSet<int32>& InSelectedUVIslandIDs, int32 InPrimarySelectedUVIslandID, bool bSyncListSelection)
{
...
FReply SWetClothingAssetEditorPanel::HandleRemoveWetPartClicked()
{
    ...
    const int32 UVChannelIndex = GetSelectedUVChannelIndex();
    WetClothingAssetPtr->Modify();
    for (int32 Index = WetClothingAssetPtr->WetPartEntries.Num() - 1; Index >= 0; --Index)
    {
        const FWetClothingAssetWetPartEntry& Entry = WetClothingAssetPtr->WetPartEntries[Index];
        ...
        {
            WetClothingAssetPtr->WetPartEntries.RemoveAt(Index);
            break;
        }
    }
    SelectedWetPartID = INDEX_NONE;
...
TMap<int32, FLinearColor> SWetClothingAssetEditorPanel::BuildAutoPartitionPreviewColorMap(const TArray<FWetPartAutoPartitionCluster>& Clusters) const
{
    TMap<int32, FLinearColor> Result;
...
void SWetClothingAssetEditorPanel::ApplyAutoPartitionClusters(const TArray<FWetPartAutoPartitionCluster>& Clusters)
{
    UWetClothingAsset* WetClothingAssetPtr = WetClothingAsset.Get();
    if (WetClothingAssetPtr == nullptr || !IsAutoPartitionEnabled())
    {
    ...
    for (int32 EntryIndex = WetClothingAssetPtr->WetPartEntries.Num() - 1; EntryIndex >= 0; --EntryIndex)
    {
        const FWetClothingAssetWetPartEntry& Entry = WetClothingAssetPtr->WetPartEntries[EntryIndex];
        if (Entry.MaterialSlotIndex == SelectedMaterialSlotIndex && Entry.UVChannelIndex == UVChannelIndex && Entry.WetPartID != 0)
        {
            WetClothingAssetPtr->WetPartEntries.RemoveAt(EntryIndex);
        }
    }
...
FReply SWetClothingAssetEditorPanel::HandleAssignSelectedUVIslandToWetPartClicked()
{
    UWetClothingAsset* WetClothingAssetPtr = WetClothingAsset.Get();
...
FText SWetClothingAssetEditorPanel::GetWetnessProfileMapBakeSourceText() const
{
...
FText SWetClothingAssetEditorPanel::GetWetnessProfileMapBakeSlotsText() const
{
...
FText SWetClothingAssetEditorPanel::GetWetnessProfileMapBakeStatusText() const
{
    UTexture* SourceTexture = ResolveSelectedMaterialTexture();
    if (SourceTexture == nullptr)
    {
    ...
    const FWetClothingAssetBakedWetnessProfileMap* BakedWetnessProfileMap = FindBakedWetnessProfileMap(SourceTexture, GetSelectedUVChannelIndex());
...
FText SWetClothingAssetEditorPanel::GetWetnessProfileMapBakeSettingsText() const
{
    const FWetClothingAssetBakedWetnessProfileMap* BakedWetnessProfileMap = FindBakedWetnessProfileMap(ResolveSelectedMaterialTexture(), GetSelectedUVChannelIndex());
...
FText SWetClothingAssetEditorPanel::GetAssignUVIslandToWetPartText() const
{
...
const FSlateBrush* SWetClothingAssetEditorPanel::GetUVSelectionToolBrush(FUVSelectionToolItemPtr Item) const
{
    ...
    {
        return FDWCEditorStyle::GetBrush(Item->IconBrushName);
    }
...
bool SWetClothingAssetEditorPanel::IsWetnessProfileMapBakeSourceValid() const
{
...
bool SWetClothingAssetEditorPanel::CanBakeAnyWetnessProfileMap() const
{
...
FReply SWetClothingAssetEditorPanel::HandleBakeSelectedWetnessProfileMapClicked()
{
    UWetClothingAsset* WetClothingAssetPtr = WetClothingAsset.Get();
    UTexture*          SourceTexture = ResolveSelectedMaterialTexture();
    ...
    const FWetClothingAssetBakedWetnessProfileMap* ExistingWetnessProfileMap = FindBakedWetnessProfileMap(SourceTexture, UVChannelIndex);
...
FReply SWetClothingAssetEditorPanel::HandleBakeAllWetnessProfileMapsClicked()
{
    UWetClothingAsset* WetClothingAssetPtr = WetClothingAsset.Get();
    if (WetClothingAssetPtr == nullptr)
    {
...
UTexture* SWetClothingAssetEditorPanel::ResolveTextureAddressTexture() const
{
...
UTexture* SWetClothingAssetEditorPanel::FindSavedTextureSelection(int32 MaterialSlotIndex, int32 UVChannelIndex) const
{
    const UWetClothingAsset* WetClothingAssetPtr = WetClothingAsset.Get();
...
bool SWetClothingAssetEditorPanel::HasSavedTextureSelection(int32 MaterialSlotIndex, int32 UVChannelIndex) const
{
    ...
    return WetClothingAssetPtr->TextureSelections.ContainsByPredicate(
        [MaterialSlotIndex, UVChannelIndex](const FWetClothingAssetTextureSelection& Selection)
        {
            return Selection.MaterialSlotIndex == MaterialSlotIndex && Selection.UVChannelIndex == UVChannelIndex;
        });
}

void SWetClothingAssetEditorPanel::SaveTextureSelection(int32 MaterialSlotIndex, int32 UVChannelIndex, UTexture* Texture)
{
    UWetClothingAsset* WetClothingAssetPtr = WetClothingAsset.Get();
    if (WetClothingAssetPtr == nullptr || MaterialSlotIndex == INDEX_NONE || UVChannelIndex == INDEX_NONE)
    {
    ...
    for (int32 SelectionIndex = WetClothingAssetPtr->TextureSelections.Num() - 1; SelectionIndex >= 0; --SelectionIndex)
    {
        FWetClothingAssetTextureSelection& Selection = WetClothingAssetPtr->TextureSelections[SelectionIndex];
        if (Selection.MaterialSlotIndex == MaterialSlotIndex && Selection.UVChannelIndex == UVChannelIndex)
        {
    ...
    WetClothingAssetPtr->TextureSelections.Add(NewSelection);
    WetClothingAssetPtr->MarkPackageDirty();
}
...
bool SWetClothingAssetEditorPanel::HasPendingWetSetupTasks(FString* OutSummary) const
{
    ...
    {
        const FWetClothingAssetWetMaterialOverride* MaterialOverride = WetClothingAssetPtr->WetMaterialOverrides.FindByPredicate(
            [MaterialSlotIndex](const FWetClothingAssetWetMaterialOverride& ExistingOverride)
            {
...
bool SWetClothingAssetEditorPanel::BuildWetSetup(FString& OutSummary, bool* OutHadWarnings)
{
    ...
    for (const int32 MaterialSlotIndex : WetMaterialSlots)
    {
        ...
        FWetClothingAssetWetMaterialOverride* ExistingOverride = WetClothingAssetPtr->WetMaterialOverrides.FindByPredicate(
            [MaterialSlotIndex](const FWetClothingAssetWetMaterialOverride& MaterialOverride)
            {
...
bool SWetClothingAssetEditorPanel::SaveWetSetupAssets() const
{
    ...
    for (const FWetClothingAssetWetMaterialOverride& MaterialOverride : WetClothingAssetPtr->WetMaterialOverrides)
    {
...
const FWetClothingAssetBakedWetnessProfileMap* SWetClothingAssetEditorPanel::FindBakedWetnessProfileMap(UTexture* SourceTexture, int32 UVChannelIndex) const
{
    const UWetClothingAsset* WetClothingAssetPtr = WetClothingAsset.Get();
    ...
    return WetClothingAssetPtr->BakedWetnessProfileMaps.FindByPredicate(
        [SourceTexture, UVChannelIndex](const FWetClothingAssetBakedWetnessProfileMap& BakedWetnessProfileMap)
        {
            return BakedWetnessProfileMap.SourceTexture == SourceTexture && BakedWetnessProfileMap.UVChannelIndex == UVChannelIndex;
        });
}

void SWetClothingAssetEditorPanel::CollectMaterialSlotsForWetnessProfileMap(UTexture* SourceTexture, int32 UVChannelIndex, TArray<int32>& OutMaterialSlotIndices) const
{
...
void SWetClothingAssetEditorPanel::CollectWetnessProfileMapSourceTextures(int32 UVChannelIndex, TArray<UTexture*>& OutSourceTextures) const
{
