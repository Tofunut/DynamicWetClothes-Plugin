...
{
	return FDynamicWetClothesEditorStyle::GetBrush(TEXT("ClassThumbnail.WetnessProfile"));
}

const FSlateBrush* UWetnessProfileAssetDefinition::GetIconBrush(const FAssetData& InAssetData, const FName InClassName) const
{
	return FDynamicWetClothesEditorStyle::GetBrush(TEXT("ClassIcon.WetnessProfile"));
}
