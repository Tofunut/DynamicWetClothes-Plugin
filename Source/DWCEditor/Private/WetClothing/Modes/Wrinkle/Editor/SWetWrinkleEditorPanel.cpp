...
namespace
{
    ...
    constexpr float WetWrinkleDefaultSizeCm = WetWrinkleBrushConstants::DefaultSizeCm;
    constexpr float WetWrinkleDefaultSizeUV = WetWrinkleBrushConstants::DefaultRadiusUV;
    constexpr float WetWrinkleUVPerCm = WetWrinkleBrushConstants::UVPerCm;
    constexpr float WetWrinkleMaxSizeCm = WetWrinkleBrushConstants::MaxSizeCm;
    constexpr float WetWrinkleMaxSizeUV = WetWrinkleBrushConstants::MaxRadiusUV;
        ...
        .Padding(10.0f)
            [SNew(SScrollBox)
             ...
             + SVerticalBox::Slot()
                   ...
                   .Padding(0.0f, 0.0f, 0.0f, 8.0f)
                       [SNew(STextBlock)
                            .Visibility(this, &SWetWrinkleEditorPanel::GetBakedTransparencyWarningVisibility)
                            .AutoWrapText(true)
                            .ColorAndOpacity(FLinearColor(1.0f, 0.65f, 0.1f))
                            .Text(this, &SWetWrinkleEditorPanel::GetBakedTransparencyWarningText)]
...
FWetWrinkleBrushSettings SWetWrinkleEditorPanel::MakeViewportBrushSettings() const
{
    ...
    PreviewSettings.PatchDiameterLocal = GetBrushSizeCm();
    if (IsUsingCustomWrinkleMap())
    {
...
void SWetWrinkleEditorPanel::HandleStrokeSelectionChanged(FStrokeListItemPtr Item, ESelectInfo::Type SelectInfo)
{
                ...
                : nullptr)
    {
        ...
        SizeCm = FMath::Clamp(SizeUV / WetWrinkleUVPerCm, 0.1f, WetWrinkleMaxSizeCm);
        SizeUV = FMath::Min(SizeUV, WetWrinkleMaxSizeUV);
        BrushSettings.BrushRadiusUV = SizeUV;
...
float SWetWrinkleEditorPanel::GetBrushSizeCm() const
{
        ...
        SelectedElementType == EWetWrinkleElementType::ProceduralRidgeStroke)
    {
        ...
        {
            return FMath::Clamp(
                Stroke->WidthUV / WetWrinkleUVPerCm,
                0.1f,
                WetWrinkleMaxSizeCm);
        }
    }
...
{
    SizeCm = FMath::Clamp(NewValue, 0.1f, WetWrinkleMaxSizeCm);
    SizeUV = FMath::Clamp(SizeCm * WetWrinkleUVPerCm, 0.001f, WetWrinkleMaxSizeUV);
    BrushSettings.BrushRadiusUV = SizeUV;
    ApplyBrushSettingsToSelectedProceduralStroke();
...
EVisibility SWetWrinkleEditorPanel::GetBakedTransparencyWarningVisibility() const
{
    if (!bShowBakedTransparency || WetClothingAsset == nullptr ||
        BrushSettings.MaterialSlotIndex == INDEX_NONE)
    {
        return EVisibility::Collapsed;
    }

    const FDWCEditorPreviewSavedLayers SavedLayers = FDWCEditorPreviewLayerResolver::Resolve(
        WetClothingAsset.Get(),
        BrushSettings.MaterialSlotIndex);
    return SavedLayers.TransparencyState == EDWCEditorSavedLayerState::Stale
        ? EVisibility::Visible
        : EVisibility::Collapsed;
}

FText SWetWrinkleEditorPanel::GetBakedTransparencyWarningText() const
{
    return LOCTEXT(
        "BakedTransparencyStaleWarning",
        "The previous Transparency Map is shown for reference. It is out of date after the wrinkle bake; rebake it in Transparency Editor before runtime use.");
}
