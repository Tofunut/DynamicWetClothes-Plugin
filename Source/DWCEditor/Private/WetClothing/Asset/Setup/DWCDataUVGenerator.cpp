    ...
    const int32 TargetMaterialSlotIndex)
{
    ...
    {
        SetFailure(Result, TEXT("Source and target LOD are the same; use full DWC Data UV generation for the canonical LOD."));
        return Result;
    }

    if (DataUVChannelIndex < 0 || DataUVChannelIndex >= 8)
    {
        SetFailure(Result, FString::Printf(TEXT("DWC Data UV channel %d is outside the supported 0-7 UV channel range."), DataUVChannelIndex));
        return Result;
    }
    ...
    {
        SetFailure(Result, FString::Printf(TEXT("Source LOD%d render data does not contain DWC Data UV channel %d."), SourceLODIndex, DataUVChannelIndex));
        return Result;
    }
    if (TargetVertexBuffer.GetNumTexCoords() <= static_cast<uint32>(DataUVChannelIndex))
    {
        ...
        {
            SetFailure(Result, FString::Printf(TEXT("Target LOD%d render data could not allocate DWC Data UV channel %d."), TargetLODIndex, DataUVChannelIndex));
            return Result;
        }
    }
    ...
    {
        SetFailure(Result, FString::Printf(TEXT("Source LOD%d has no valid triangles for DWC Data UV transfer."), SourceLODIndex));
        return Result;
    }
    ...
    {
        SetFailure(Result, FString::Printf(TEXT("Target LOD%d has no render vertices that could receive transferred DWC Data UVs."), TargetLODIndex));
        return Result;
    }
    ...
    Result.Message = FString::Printf(
        TEXT("Transferred DWC Data UV channel %d from LOD%d to LOD%d render data for %d render vertex/vertices."),
        DataUVChannelIndex,
        SourceLODIndex,
        TargetLODIndex,
        WrittenVertexCount);
