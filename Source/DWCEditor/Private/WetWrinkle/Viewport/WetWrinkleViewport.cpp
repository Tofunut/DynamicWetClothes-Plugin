...
void SWetWrinkleViewport::RefreshWrinklePreviewMaterials()
{
    ...
    if (ActiveMaterialSlotIndex != INDEX_NONE)
    {
        ...
        if (PreviewMaterialSlots.IsValidIndex(ActiveMaterialSlotIndex))
        {
            ...
            {
                if (UTexture* SourceTexture = ResolveSourceTextureForMaterialSlot(ActiveMaterialSlotIndex, BrushSettings.UVChannelIndex))
                {
                    if (UTexture2D* AccumulatedPreviewTexture = ResolveAccumulatedPreviewTexture(SourceTexture, BrushSettings.UVChannelIndex))
                    {
...
{
    if (Stamp.SourceTexture == nullptr || Stamp.UVChannelIndex < 0)
    {
        return;
    }

    FWetWrinkleAccumulatedPreviewState* PreviewState = FindOrAddAccumulatedPreviewState(Stamp.SourceTexture.Get(), Stamp.UVChannelIndex);
...
FWetWrinkleAccumulatedPreviewState* SWetWrinkleViewport::FindOrAddAccumulatedPreviewState(UTexture* SourceTexture, int32 UVChannelIndex)
{
    if (SourceTexture == nullptr || UVChannelIndex < 0)
    {
        return nullptr;
    }
    ...
    for (FWetWrinkleAccumulatedPreviewState& PreviewState : AccumulatedPreviewStates)
    {
        if (PreviewState.SourceTexture == SourceTexture && PreviewState.UVChannelIndex == UVChannelIndex)
        {
            return &PreviewState;
        }
    }
...
UTexture2D* SWetWrinkleViewport::ResolveAccumulatedPreviewTexture(UTexture* SourceTexture, int32 UVChannelIndex)
{
    FWetWrinkleAccumulatedPreviewState* PreviewState = FindOrAddAccumulatedPreviewState(SourceTexture, UVChannelIndex);
    if (PreviewState == nullptr)
    {
        return nullptr;
    }
...
bool SWetWrinkleViewport::RebuildAccumulatedPreviewTexture(FWetWrinkleAccumulatedPreviewState& PreviewState)
{
    ...
    if (PreviewState.SourceTexture == nullptr || PreviewState.UVChannelIndex < 0)
    {
    ...
    if (Asset != nullptr)
    {
        for (const FWetWrinklePatchStroke& Stroke : Asset->WrinkleData.EditablePatchStrokes)
        {
            ...
            {
                if (Stamp.SourceTexture != PreviewState.SourceTexture || Stamp.UVChannelIndex != PreviewState.UVChannelIndex)
                {
                    continue;
                }
