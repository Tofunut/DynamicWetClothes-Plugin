...
FDWCDataUVGenerationResult FDWCDataUVGenerator::GenerateForSkeletalMeshRenderData(
    const USkeletalMesh* SkeletalMesh,
    int32 LODIndex,
    ...
    int32 TargetMaterialSlotIndex)
{
    ...
    {
        SetFailure(Result, FString::Printf(TEXT("LOD %d render data is unavailable."), LODIndex));
        return Result;
    }

    const FSkeletalMeshLODRenderData& LODData = RenderData->LODRenderData[LODIndex];
    const int32 VertexCount = static_cast<int32>(LODData.GetNumVertices());
    const int32 NumTexCoords = static_cast<int32>(LODData.GetNumTexCoords());
    const int32 SafeSourceUVChannelIndex = FMath::Clamp(SourceUVChannelIndex, 0, 7);
    if (VertexCount <= 0)
    {
        SetFailure(Result, FString::Printf(TEXT("LOD %d has no render vertices."), LODIndex));
        return Result;
    }
    if (SafeSourceUVChannelIndex >= NumTexCoords)
    {
        SetFailure(Result, FString::Printf(TEXT("Source UV Channel %d does not exist on LOD %d."), SafeSourceUVChannelIndex, LODIndex));
        return Result;
    }
    ...
    {
        SetFailure(Result, FString::Printf(TEXT("LOD %d index buffer is empty."), LODIndex));
        return Result;
    }

    TArray<FTriangleRecord> Triangles;
    TMap<int32, TArray<int32>> SlotToTriangleIndices;
    TSet<int32> ExcludedVertexIDs;
    ...
    for (const FSkelMeshRenderSection& Section : LODData.RenderSections)
    {
        ...
        for (int32 IndexOffset = FirstIndex; IndexOffset + 2 < LastIndex; IndexOffset += 3)
        {
            const int32 VertexIndices[3] = {
                static_cast<int32>(IndexBuffer[IndexOffset]),
                static_cast<int32>(IndexBuffer[IndexOffset + 1]),
                static_cast<int32>(IndexBuffer[IndexOffset + 2])};
            if (VertexIndices[0] < 0 || VertexIndices[0] >= VertexCount ||
                VertexIndices[1] < 0 || VertexIndices[1] >= VertexCount ||
                VertexIndices[2] < 0 || VertexIndices[2] >= VertexCount)
            {
                continue;
            }

            FTriangleRecord Triangle;
            Triangle.TriangleID = FTriangleID(IndexOffset / 3);
            Triangle.MaterialSlotIndex = MaterialSlotIndex;
            for (int32 CornerIndex = 0; CornerIndex < 3; ++CornerIndex)
            {
                const int32 VertexIndex = VertexIndices[CornerIndex];
                Triangle.VertexInstances[CornerIndex] = FVertexInstanceID(VertexIndex);
                Triangle.Vertices[CornerIndex] = FVertexID(VertexIndex);
                Triangle.Positions[CornerIndex] = FVector(LODData.StaticVertexBuffers.PositionVertexBuffer.VertexPosition(VertexIndex));
                const FVector2f SourceUV = LODData.StaticVertexBuffers.StaticMeshVertexBuffer.GetVertexUV(VertexIndex, SafeSourceUVChannelIndex);
            ...
            if (!bSourceUVIsFinite)
            {
                ...
                {
                    ExcludedVertexIDs.Add(VertexInstanceID.GetValue());
                }
                continue;
            }
    ...
    {
        SetFailure(Result, TargetMaterialSlotIndex != INDEX_NONE
                               ? FString::Printf(TEXT("Material Slot %d does not contain triangles that can be unwrapped."), TargetMaterialSlotIndex)
                               : FString(TEXT("The target mesh does not contain triangles that can be unwrapped.")));
        return Result;
    }
    ...
    for (const int32 MaterialSlotIndex : SortedSlotIndices)
    {
        if (const TArray<int32>* SlotTriangleIndices = SlotToTriangleIndices.Find(MaterialSlotIndex))
        {
            BuildConnectedIslandsForSlot(Triangles, *SlotTriangleIndices, SourceIslands);
        }
    }

    if (SourceIslands.IsEmpty())
    {
    ...
    TMap<int32, FVector2f> PackedUVByVertex;
    PackIslandsIntoUnitSquare(Triangles, Islands, InternalPackingResolution, InternalPaddingPixels, PackedUVByVertex);

    TSet<int32> ProblemMaterialSlots;
    FString PackedValidationError;
    if (!ValidatePackedUVLayout(Triangles, Islands, PackedUVByVertex, ProblemMaterialSlots, PackedValidationError))
    {
        ...
        if (!ValidatePackedUVLayout(Triangles, Islands, PackedUVByVertex, ProblemMaterialSlots, PackedValidationError))
        {
            SetFailure(Result, FString::Printf(TEXT("DWC Data UV generation failed final non-overlap validation: %s"), *PackedValidationError));
            return Result;
        }
    }

    Result.DataUVs.Init(FVector2f::ZeroVector, VertexCount);
    for (const TPair<int32, FVector2f>& Pair : PackedUVByVertex)
    {
        if (Result.DataUVs.IsValidIndex(Pair.Key))
        {
            Result.DataUVs[Pair.Key] = Pair.Value;
        }
    }

    Result.bSucceeded = true;
    Result.UVChannelIndex = INDEX_NONE;
    Result.MaterialSlotIndex = TargetMaterialSlotIndex;
    Result.RenderVertexCount = VertexCount;
    ...
    Result.Message = FString::Printf(
        TEXT("Generated WCA DWC Data UVs for LOD%d %s with %d render vertices and %d packed source UV island(s)."),
        LODIndex,
        *TargetLabel,
        VertexCount,
        Islands.Num());
    ...
    {
        Result.Message += FString::Printf(
            TEXT(" Warnings: excluded %d degenerate source-UV triangle(s) and %d invalid source-UV triangle(s); split %d self-overlapping source island(s) across %d overlap pair(s); triangle fallback charts: %d."),
            Result.DegenerateSourceUVTriangleCount,
            Result.InvalidSourceUVTriangleCount,
            Result.SplitSourceIslandCount,
            Result.SelfOverlapPairCount,
