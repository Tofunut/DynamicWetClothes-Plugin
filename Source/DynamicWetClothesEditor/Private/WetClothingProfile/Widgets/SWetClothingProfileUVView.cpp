...
{
	return SelectionTool == EWetClothingProfileUVSelectionTool::BoxSelect
		|| SelectionTool == EWetClothingProfileUVSelectionTool::EllipseSelect
		|| SelectionTool == EWetClothingProfileUVSelectionTool::LassoSelect;
}
	...
	const FVector2D& EllipseRadii) const
{
	...
	return DoesSegmentIntersectEllipse(Triangle.UVs[0], Triangle.UVs[1], EllipseCenter, EllipseRadii)
		|| DoesSegmentIntersectEllipse(Triangle.UVs[1], Triangle.UVs[2], EllipseCenter, EllipseRadii)
		|| DoesSegmentIntersectEllipse(Triangle.UVs[2], Triangle.UVs[0], EllipseCenter, EllipseRadii);
}
	...
	const FVector2D& BEnd) const
{
	...
	{
		return Point.X >= FMath::Min(SegmentStart.X, SegmentEnd.X) - KINDA_SMALL_NUMBER
			&& Point.X <= FMath::Max(SegmentStart.X, SegmentEnd.X) + KINDA_SMALL_NUMBER
			&& Point.Y >= FMath::Min(SegmentStart.Y, SegmentEnd.Y) - KINDA_SMALL_NUMBER
			&& Point.Y <= FMath::Max(SegmentStart.Y, SegmentEnd.Y) + KINDA_SMALL_NUMBER;
	};
