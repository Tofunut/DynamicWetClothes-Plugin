...
{
#if WITH_EDITOR
    static const TCHAR* DWCGeneratedAssetOwnerGuidMetadataKey = TEXT("DWC.GeneratedAssetOwnerGuid");
#endif
    ...
    FString MakeWetnessProfileParametersHash(const FWetnessProfileParameters& Parameters)
    {
        ...
        const FString SurfaceKeyHead = FString::Printf(
            TEXT("Surf{%d,Secondary{%d},Dry{%.9g},D1{%.9g,%.9g,%.9g},"),
        ...
        const FString SurfaceKeyTail = FString::Printf(
            ...
            *MakeTextureContentIdentity(Surface.DropletNormalTexture.Get()),
            *MakeTextureContentIdentity(Surface.DropletMaskTexture.Get()),
            ...
            *MakeTextureContentIdentity(Surface.DropletFlowNormalTexture.Get()),
            *MakeTextureContentIdentity(Surface.DropletFlowMaskTexture.Get()),
        ...
        for (const int32 SlotIndex : SlotIndices)
        {
            ...
            for (const int32 EntryIndex : EntryIndices)
            {
                ...
                Signature += FString::Printf(
                    ...
                    Profile != nullptr ? *MakeWetnessProfileParametersHash(Profile->Parameters) : TEXT(""),
        ...
        FString* OutErrorMessage)
    {
        ...
        const int32 SurfaceNormalUVChannel =
            Settings.SurfaceWaterNormalUVChannelIndex != INDEX_NONE
                ? Settings.SurfaceWaterNormalUVChannelIndex
                : Settings.OriginalUVChannelIndex;
        if (SurfaceNormalUVChannel < 0 || SurfaceNormalUVChannel >= UVChannelCount)
        {
            DWC::Error::SetMessage(
                OutErrorMessage,
                FString::Printf(
                    TEXT("Surface Normal UV Channel UV%d is unavailable on LOD%d. That LOD has %d UV channel(s)."),
                    SurfaceNormalUVChannel,
                    SourceLODIndex,
                    UVChannelCount));
            return false;
        }
        ...
        if (Settings.PreferredDWCDataUVChannelIndex == SurfaceNormalUVChannel)
        {
            DWC::Error::SetMessage(
                OutErrorMessage,
                FString::Printf(
                    TEXT("Surface Normal UV cannot use UV%d because that channel is the DWC UV destination and will be overwritten by packed data UVs."),
                    SurfaceNormalUVChannel));
            return false;
        }
        return true;
...
void UWetClothingAsset::PostLoad()
{
    ...
    for (FWetPartProfileAssignment& ProfileAssignment : EditableWetPartData.Profiles)
    {
        ProfileAssignment.MigrateLegacySoftReference();
    }
    for (FWetClothingLocalRenderProfile& LocalProfile : Derived.Inline.BakedWetPartData.LocalProfiles)
    {
        LocalProfile.MigrateLegacySoftReferences();
    }
    ...
    EnsureAssetGuid();
#if WITH_EDITORONLY_DATA
    Derived.Inline.PreviewTopologyRevision = FMath::Max<uint64>(
        Derived.Inline.PreviewTopologyRevision,
        1);
    Metadata.GeneratedAssetManifest.RemoveAll(
        [](const TSoftObjectPtr<UObject>& Entry)
        {
            return Entry.IsNull();
        });
#endif
    auto AdoptLegacyGeneratedReference = [this](UObject* GeneratedAsset)
    {
        ...
        {
            // A serialized hard reference is authoritative for legacy assets that predate ownership tags.
            TagGeneratedAsset(GeneratedAsset);
        }
        else if (ExistingOwnerGuid != Metadata.AssetGuid)
        {
            UE_LOG(
                LogDWC,
                Warning,
                TEXT("WetClothingAsset: '%s' references generated asset '%s', but that asset is owned by another WCA. The reference is retained for inspection and will not be retagged automatically."),
                *GetNameSafe(this),
                *GetPathNameSafe(GeneratedAsset));
        }
    };

    AdoptLegacyGeneratedReference(Metadata.DWCSkeletalMesh);
    for (const FWetClothingGeneratedWetMaterialOverride& Override : Derived.Inline.GeneratedWetMaterialOverrides)
    {
        AdoptLegacyGeneratedReference(Override.GeneratedMaterial.Get());
        AdoptLegacyGeneratedReference(Override.GeneratedMaterialInstance.Get());
    }
    for (const FWetClothingBakedWetPartDataSlotTexture& SlotTexture : Derived.Inline.BakedWetPartData.SlotTextures)
    {
        AdoptLegacyGeneratedReference(SlotTexture.WetPartDataTexture.Get());
    }
    for (const FWetWrinkleBakedMapSet& BakedMap : Authored.WrinkleData.BakedWrinkleMaps)
    {
        AdoptLegacyGeneratedReference(BakedMap.BakedWrinkleNormalMap.Get());
#if WITH_EDITORONLY_DATA
        AdoptLegacyGeneratedReference(BakedMap.BakedWrinkleMask.Get());
#endif
    }
    for (const FWetClothingTransparencyLayerData& Layer : Authored.TransparencyData.TransparencyLayers)
    {
        for (const FWetClothingBakedTransparencyMap& BakedMap : Layer.BakedMaps)
        {
            AdoptLegacyGeneratedReference(BakedMap.TransparencyMap.Get());
        }
    }
#endif
...
bool UWetClothingAsset::TagGeneratedAsset(UObject* GeneratedAsset)
{
        ...
        ExistingOwnerGuid != Metadata.AssetGuid)
    {
        UE_LOG(
            LogDWC,
            Warning,
            TEXT("WetClothingAsset: Refused to retag generated asset '%s' from owner %s to WCA '%s' (%s)."),
            *GetPathNameSafe(GeneratedAsset),
    ...
    MetaData.SetValue(
        GeneratedAsset,
        DWCGeneratedAssetOwnerGuidMetadataKey,
        *Metadata.AssetGuid.ToString(EGuidFormats::DigitsWithHyphens));
#if WITH_EDITORONLY_DATA
    Metadata.GeneratedAssetManifest.AddUnique(TSoftObjectPtr<UObject>(GeneratedAsset));
#endif
    GeneratedAsset->MarkPackageDirty();
    MarkPackageDirty();
    return true;
}
    ...
    FGuid& OutOwnerGuid) const
{
    ...
    FMetaData& MetaData = Package->GetMetaData();
    const FString StoredGuid = MetaData.GetValue(
        GeneratedAsset,
        DWCGeneratedAssetOwnerGuidMetadataKey);
    return FGuid::Parse(StoredGuid, OutOwnerGuid) && OutOwnerGuid.IsValid();
}
...
bool UWetClothingAsset::IsGeneratedAssetOwnedByThisWCA(const UObject* GeneratedAsset) const
{
    ...
    FGuid StoredOwnerGuid;
    return TryGetGeneratedAssetOwnerGuid(GeneratedAsset, StoredOwnerGuid) &&
        StoredOwnerGuid == Metadata.AssetGuid;
}
    ...
    UClass* RequiredClass) const
{
    OutAssets.Reset();
#if WITH_EDITORONLY_DATA
    for (const TSoftObjectPtr<UObject>& AssetReference : Metadata.GeneratedAssetManifest)
    {
        UObject* Asset = AssetReference.Get();
        if (Asset == nullptr && !AssetReference.IsNull())
        {
            Asset = AssetReference.LoadSynchronous();
        }
        if (Asset == nullptr ||
            (RequiredClass != nullptr && !Asset->IsA(RequiredClass)) ||
            !IsGeneratedAssetOwnedByThisWCA(Asset))
        {
            continue;
        }
...
void UWetClothingAsset::RemoveGeneratedAssetFromManifest(const UObject* GeneratedAsset)
{
    ...
    Metadata.GeneratedAssetManifest.RemoveAll(
        ...
        {
            return Entry.ToSoftObjectPath() == RemovedPath;
        });
#endif
}
...
#if WITH_EDITORONLY_DATA
    Derived.Inline.PreviewTopologyRevision = FMath::Max<uint64>(
        Derived.Inline.PreviewTopologyRevision + 1,
        1);
#endif
}
    ...
    FString* OutErrorMessage)
{
    ...
    for (int32 MaterialSlotIndex = 0; MaterialSlotIndex < InSourceMesh->GetMaterials().Num(); ++MaterialSlotIndex)
    {
        ...
        // Legacy per-slot override is intentionally unused by the simplified
        // global Surface Normal UV policy. Keep it unset for serialized compatibility.
        Slot.SurfaceWater.SurfaceWaterNormalUVChannel = INDEX_NONE;
    ...
    FString* OutChangeSummary)
{
    ...
    const bool bSurfaceNormalUVChanged =
        PreviousSettings.SurfaceWaterNormalUVChannelIndex !=
        NewSettings.SurfaceWaterNormalUVChannelIndex;
    ...
    if (bSurfaceNormalUVChanged)
    {
        // The GPU triangle payload stores DataUV -> SurfaceNormalUV transforms.
        // Only GPU runtime/map data is stale; CPU topology and DWC Data UV remain valid.
        MarkRuntimeBakeOutputsDirty(
            DWCBakeOutput::GPURuntimeData |
            DWCBakeOutput::GPUMaps);
    }
    ...
    if (bSurfaceNormalUVChanged)
    {
        const int32 PreviousResolved =
            PreviousSettings.SurfaceWaterNormalUVChannelIndex != INDEX_NONE
                ? PreviousSettings.SurfaceWaterNormalUVChannelIndex
                : PreviousSettings.OriginalUVChannelIndex;
        const int32 CurrentResolved = GetSurfaceWaterNormalUVChannelIndex();
        Changes.Add(FString::Printf(
            TEXT("Surface Normal UV changed: UV%d -> UV%d."),
            PreviousResolved,
            CurrentResolved));
    }
    ...
    FString* OutErrorMessage)
{
    ...
    TagGeneratedAsset(InRuntimeMesh);
