...
TSharedRef<SWidget> FWCAEditorWidgets::BuildRuntimeBuildMenu(const FWCARuntimeBuildMenuArgs& Args)
{
    ...
    const auto BuildStatusTooltip = [&Args](const FDWCEditorBuildActionDescriptor& Descriptor)
    {
        const FDWCEditorBuildActionStatus* Status = Args.Snapshot.Find(Descriptor.Action);
        if (Status == nullptr || Status->Reason.IsEmpty())
        {
            return Descriptor.Description;
        }
        return FText::Format(
            NSLOCTEXT("WetClothingEditorCommonWidgets", "BuildActionTooltipWithStatus", "{0}\n\nStatus: {1}"),
            Descriptor.Description,
            FText::FromString(Status->Reason));
    };
    ...
    MenuBuilder.AddMenuEntry(
        ...
        }), FCanExecuteAction::CreateLambda([Plan = Args.RequiredPlan]()
        {
            return Plan.IsExecutable() && !Plan.Steps.IsEmpty();
        })));
    MenuBuilder.EndSection();

    FName OpenSection = NAME_None;
    for (const FDWCEditorBuildActionDescriptor& Descriptor :
         FDWCEditorBuildActionRegistry::GetDescriptors())
    {
        if (!Descriptor.bShowInRuntimeMenu ||
            (Descriptor.VisibleSurfaceMode != EDWCEditorBuildSurfaceMode::Any &&
             Descriptor.VisibleSurfaceMode != Args.SurfaceMode))
        {
            continue;
        }
        if (OpenSection != Descriptor.MenuSection)
        {
            if (!OpenSection.IsNone())
            {
                MenuBuilder.EndSection();
            }
            OpenSection = Descriptor.MenuSection;
            MenuBuilder.BeginSection(
                OpenSection,
                FText::FromName(OpenSection).ToUpper());
        }

        const FDWCEditorBuildActionStatus* Status = Args.Snapshot.Find(Descriptor.Action);
        const bool bCanExecute = Status != nullptr && Status->IsExecutable();
        const FName IconStyleSet = Descriptor.IconStyleSetName.IsNone()
            ? FAppStyle::GetAppStyleSetName()
            : Descriptor.IconStyleSetName;
        MenuBuilder.AddMenuEntry(
            Descriptor.DisplayName,
            BuildStatusTooltip(Descriptor),
            FSlateIcon(IconStyleSet, Descriptor.IconName),
            FUIAction(
                FExecuteAction::CreateLambda(
                    [OnExecuteAction = Args.OnExecuteAction, Action = Descriptor.Action]()
                    {
                        if (OnExecuteAction)
                        {
