...
namespace DWCDataUVGeneratorInternal
{
    ...
    static constexpr int32  SupportedUVChannelCount = SupportedUVChannelMaxIndex + 1;
    ...
    {
        if (RequiredUVChannelIndex < 0 || RequiredUVChannelIndex > SupportedUVChannelMaxIndex)
        {
            return false;
        }
    ...
    const bool                bAllowVisibleExclusionAboveSafetyLimit)
{
    ...
    if (SourceUVChannelIndex < 0 || SourceUVChannelIndex > SupportedUVChannelMaxIndex)
    {
        SetFailure(Result, TEXT("Source UV Channel must be between UV0 and UV3."));
        return Result;
    }
    const int32 SafeSourceUVChannelIndex = SourceUVChannelIndex;
    ...
    if (PreferredUVChannelIndex < 0 || PreferredUVChannelIndex > SupportedUVChannelMaxIndex)
    {
        SetFailure(Result, TEXT("DWC UV Channel must be between UV0 and UV3."));
        return Result;
    }
    const int32 SafePreferredUVChannelIndex = PreferredUVChannelIndex;
    ...
    {
        if (ExistingUVChannelCount >= SupportedUVChannelCount)
        {
            SetFailure(Result, FString::Printf(
                                   TEXT("UV Channel %d already exists and is not marked as generated by DWC. UV0-UV3 are already occupied, so a new supported DWC UV Channel cannot be appended."),
    ...
    for (const int32 MaterialSlotIndex : DiagnosticSlotIndices)
    {
        ...
        {
            // The single user-facing safety threshold is the slot's total excluded
            ...
            {
                // A usable layout can still be generated by excluding the problematic region.
                // Require explicit editor confirmation instead of converting the slot to Failed.
                SlotDiagnostic.bVisibleExclusionSafetyLimitExceeded = true;
                Severity = EDWCDataUVResultSeverity::ReadyWithWarnings;
                if (!bAllowVisibleExclusionAboveSafetyLimit)
                {
                    Result.ConfirmationRequiredMaterialSlotIndices.Add(MaterialSlotIndex);
                }
            }
        }
    ...
    const bool                      bDeferMeshCommit)
{
    ...
    if (Plan.UVChannelIndex < 0 || Plan.UVChannelIndex > SupportedUVChannelMaxIndex)
    {
    ...
    const int32    TargetMaterialSlotIndex)
{
    ...
    {
        SetFailure(Result, FString::Printf(TEXT("DWC UV Channel %d is outside the supported 0-3 UV channel range."), DataUVChannelIndex));
        return Result;
    }
