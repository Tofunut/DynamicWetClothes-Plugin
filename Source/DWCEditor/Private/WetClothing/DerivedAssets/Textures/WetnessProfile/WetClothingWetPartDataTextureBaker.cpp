    ...
    FString&              OutErrorMessage)
{
    ...
    {
        ExistingTexture = ExistingSlot->WetPartDataTexture.Get();
    }

    const bool bExistingMatchesTarget =
        IsValid(ExistingTexture) && ExistingTexture->GetPathName() == ObjectPath;
    FDWCEditorArtifactTextureRequest Request;
    Request.OwnerAsset = &WetClothingAsset;
    Request.PackageName = PackageName;
    Request.AssetName = ObjectName;
    Request.ExistingTexture = bExistingMatchesTarget ? ExistingTexture : nullptr;
    Request.bExistingReferenceIsTrusted = bExistingMatchesTarget;
    Request.Resolution = FIntPoint(Width, Height);
    ...
    Request.Settings.CompressionSettings = TC_VectorDisplacementmap;
    Request.Settings.Filter = TF_Nearest;
    Request.Settings.bNeverStream = true;
    Request.DebugName = FString::Printf(TEXT("Wet Part data slot %d"), MaterialSlotIndex);
    ...
    if (!FDWCEditorArtifactStore::Get()->CommitTextureBatch(
            Requests, Receipts, OutErrorMessage) || Receipts.Num() != 1)
    {
        return nullptr;
    }
    OutErrorMessage.Reset();
    return Receipts[0].Texture;
#else
