...
namespace
{
    ...
    FDWCTransparencySourcePayload BuildHoverTestResult(const FIntPoint Resolution)
    {
        ...
        for (int32 Y = 0; Y < Resolution.Y; ++Y)
        {
            for (int32 X = 0; X < Resolution.X; ++X)
            {
                ...
                Result.OuterIslandIDBuffer[Index] =
                    FDWCTransparencySourcePayload::EncodeOuterIslandID(IslandID);
            }
        }
        return Result;
    }
    ...
    float ResolveAlpha(const FDWCTransparencySourcePayload& Result,
        ...
        const int32 IslandID)
    {
        ...
        for (int32 OffsetY = -1; OffsetY <= 1; ++OffsetY)
        {
            for (int32 OffsetX = -1; OffsetX <= 1; ++OffsetX)
            {
                ...
                if (IsIslandPixel(Result, SampleIndex, IslandID))
                {
...
bool FDWCTransparencyMaterialHoverAlphaParityTest::RunTest(const FString&)
{
    ...
    const FDWCTransparencySourcePayload Result = BuildHoverTestResult(Resolution);
    ...
    for (const EDWCTransparencyBrushMode Mode : Modes)
    {
        ...
        const int32 ResolvedIslandID = Result.ResolveOuterIslandIDAtUV(
            Sample.PositionUV,
            Sample.UVIslandID,
            bWrap);
        ...
        {
            TestEqual(
                TEXT("The wrapped fixture resolves the raster-space island at the brush center"),
                ResolvedIslandID,
                HoverTestIsland + 1);
        }
        for (int32 Y = 0; Y < Resolution.Y; ++Y)
        {
            for (int32 X = 0; X < Resolution.X; ++X)
            {
                ...
                float Weight = IsIslandPixel(Result, PixelIndex, ResolvedIslandID)
                ...
                {
                    Target = ComputeSmoothAlpha(Result, Before, X, Y, bWrap, ResolvedIslandID);
                }
    ...
    for (const EDWCTransparencyRevealColorBrushMode Mode : Modes)
    {
        ...
        for (int32 Y = 0; Y < Resolution.Y; ++Y)
        {
            for (int32 X = 0; X < Resolution.X; ++X)
            {
                ...
                const float Weight = IsIslandPixel(Result, PixelIndex, HoverTestIsland)
                    ...
                    for (int32 OffsetY = -1; OffsetY <= 1; ++OffsetY)
                    {
                        for (int32 OffsetX = -1; OffsetX <= 1; ++OffsetX)
                        {
                            ...
                            Target += FLinearColor(Before.GetColor(
                                IsIslandPixel(Result, NeighborIndex, HoverTestIsland) ? NeighborIndex : PixelIndex,
